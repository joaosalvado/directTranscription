...
int main() {
    ...
    double L = 0.2;
    casadi::Opti ocp;
    DM x0 = DM::vertcat({ 2, 2, -M_PI });
    DM xf = DM::vertcat({ 1, 1, 1.5*M_PI});
    Slice all;
    ...
    // 1.4 - Dynamics Model
    SX X_dot = SX::vertcat(
            {u->v_ode *(1 - tan(x->o_ode/2)*tan(x->o_ode/2) )/(1+tan(x->o_ode/2)*tan(x->o_ode/2)),
             u->v_ode * (2 * tan(x->o_ode/2))/(1+tan(x->o_ode/2)*tan(x->o_ode/2)),
             (1 / L) * u->w_ode});
